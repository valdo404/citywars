...
use crate::app::components::setup_webgpu::setup_webgpu;
    ...
    use_effect(move || {
        ...
        wasm_bindgen_futures::spawn_local(async move {
            if let Some(canvas) = canvas_ref.read().clone() {
                debug!("Canvas found - initializing WebGPU");
                // Hand off to setup_webgpu for real initialization
                setup_webgpu(canvas).await;
            } else {
